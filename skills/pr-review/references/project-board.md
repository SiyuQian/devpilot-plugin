...
If the review aborts after a successful claim, make one best-effort recovery call:

```bash
"$REVIEW_BOARD" set --pr <pr-url> --status "Waiting to be picked up"
```

Do not transition local diffs, pasted patches, GitLab merge requests, or closed/merged PRs.
